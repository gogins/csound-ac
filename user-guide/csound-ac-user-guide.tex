...
Copy  \lstinline{\_CsoundAC.so} and \lstinline{Csound.py}, as well as \lstinline{GeneralizedContextualGroup.py}, to the \lstinline{site-packages} directory for that version, e.g. on macOS it could be \lstinline|/opt/homebrew/lib/python3.12/site-packages/_CsoundAC.so|.
...
Thus, code for running Csound is not needed, as Csound is already running; instead, the generated score is written to a temporary filename and read back into Csound for performance. 
...
\item Running in Python: \lstinline|csoundac-basic-python.py|.

\item Running in Csound: \lstinline|csoundac-basic-score-bin.csd|.

\item Running in Reaper: \lstinline|csoundac-basic-reascript.py|.

\end{enumerate}

\subsection{Compositional Examples}

These three examples are more complex, and demonstrate typical uses of CsoundAC. Two implementations are provided for each example, for standalone Pytghon and as Python ReaScript for Reaper.

\subsubsection{Transforming Source Material: \lstinline|python-trans-scale.py|}
...
\item Repeat that transformed scale three times, as a canon at the fifth.
\item Repeat that canon, and apply to it randomized chord progressions and modulations.
\end{enumerate}

\subsubsection{Translating Images to Scores:  \lstinline|python-imagery.py|}
