...
CsoundAC is a class library and software toolkit designed to support the algorithmic composition of music. CsoundAC has C++, Python, and JavaScript application programming interfaces (APIs). This \emph{\textbf{Guide}} is based on the Python interface. The major features of CsoundAC are:
...
Reference documentation for CsoundAC is \href{https://github.com/gogins/csound-ac/blob/master/csound-ac.pdf}{here}.
...
Copy  \texttt{\_CsoundAC.so} and \texttt{Csound.py}, as well as \texttt{GeneralizedContextualGroup.py}, to the \texttt{site-packages} directory for that version, e.g. on macOS it could be \lstinline|/opt/homebrew/lib/python3.12/site-packages/_CsoundAC.so|.
...
\subsubsection{Confiiguring ctcsound}
...
\begin{enumerate}
\item Two Dualities, a Lindenmayer system based on the Generalized Contextual Group of Fiore and Satyendra.
\item Chaos.
\item Image to score.
\end{enumerate}
\end{enumerate}
...
For insight into the code, open the code in an editor and read it, there are many explanatory comments.

\subsubsection{Running in Python: \lstinline|basic-python.py|}

Running in Python has the advantage of being easy to understand. A composition is just a Python program. This example uses ctcsound to perform the example.

Open a terminal, change to the \lstinline|csound-ac/user-guide| directory, and execute \lstinline|python3.12 basic-python.py|

\subsubsection{Running in Csound: \lstinline|basic-score-bin.csd|}
...
Thus, code for running Csound is not needed, as Csound is already running; instead, the generated score is written to a generated filename and read back into Csound for performance. 

Open a terminal, change to the \lstinline|csound-ac/user-guide| directory, and execute \lstinline|csound basic-score-bin.csd|.

\subsubsection{Running in Reaper: \lstinline|basic-reascript.py|}

Running in Reaper has the advantages of automatically providing a piano roll score, or even music notation, for generated scores; enabling other synthesis programs besides Csound to render CsoundAC scores; and in general, integrating with contemporary music production practice. The Csound orchestra is defined in the state of a \href{https://github.com/gogins/csound-vst3}{CsoundVST3} plugin, which receives MIDI input from the DAW. The compositional code is defined in a Python ReaScript file, which is loaded into Reaper as an Action. This separation of concerns between composition and synthesis enables CsoundAC to send MIDI to the DAW and to other plugins, and for Csound to receive MIDI and audio from any track or plugin in Reaper.
...
These three examples re-create the three basic examples above, changing them only to generate the ascending chromatic scale as a CsoundAC Score object, and to render the Score using CsoundAC's integration with Csound.

\subsubsection{Running in Python: \lstinline|csoundac-basic-python.py|}

\subsubsection{Running in Csound: \lstinline|csoundac-basic-score-bin.csd|}

\subsubsection{Running in Reaper: \lstinline|csoundac-basic-reascript.py|}

\subsection{Compositional Examples}

These three examples consist of actual, albeit simple, compositions that demonstrate typical uses of CsoundAC.

\subsubsection{Transforming Source Material \lstinline|TransScale.csd|}

This piece takes the ascending chromatic scale of the previous examples, and uses CsoundAC to transform it. 
...
\item Repeat transformed scale three times, as a canon at the fifth.
\ttem Repeat that canon, and apply to it randomized chord progressions and modulations.
\end{enumerate}

\subsubsection{Translating Images to Scores  \lstinline|basic-score-bin.csd|}

This piece takes a high-resolution photograph, applies some visual processing, translates the processed image to a CsoundAC score, and renders it using Csound.

\subsubsection{Lindenmayer Systems in Chord Space \lstinline|basic-score-bin.csd|}
