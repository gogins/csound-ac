...
For better understanding of how CsoundAC works:

\begin{itemize}
\item A musical composition is contained in an instance of the MusicModel class, which also contains a Csound orchestra.
\item A MusicModel is a directed acyclical graph, or tree, of Nodes. Some Nodes generate Events (usually notes), and other Nodes transform Events.
\item Each Node has its own local transformation of coordinate system. By default, this is the identity matrix.
\item When a score is generated, the tree of Nodes is traversed from the bottom up to the top (depth-first traversal). The traversal function carries along a global transformation of coordinate system, and a Score for collecting any Events produced or transformed by child nodes.
\item Each node applies its local transformation of coordinate system to the global coordinate system, and the result is applied to generated or transformed Events, which are then added to the global Score.
\item Once the global Score has been generated, the MusicModel uses an embedded instance of Csound to compile its orchestra, translates the global CsoundAC Score to a Csound score, and renders that score with the orchestra.
\end{itemize}
...
\item Call ImageToScore.generateLocally to translate  the brighter features to CsoundAC Events (notes). Hue (color) is instrument, and Value (brightness) is loudness. Saturation is not used.
...
\item Call MusicModel.generate to the generate the score, applying each node to the notes produced by its children.
\item Set the final generateed score to the desired length.
\item Call MusicModel.perform to render the score using the Csound orchestra.
