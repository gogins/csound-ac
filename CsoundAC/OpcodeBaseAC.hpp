...
static inline int csound_opcode_reinit_flag(const OPDS &opds)
{
    return static_cast<int>(opds.insdshead->reinitflag);
}

static inline int csound_opcode_tie_flag(const OPDS &opds)
{
    return static_cast<int>(opds.insdshead->tieflag);
}
#else
static inline int csound_opcode_reinit_flag(CSOUND *csound)
{
    return csound->GetReinitFlag(csound);
}

static inline int csound_opcode_tie_flag(CSOUND *csound)
{
    return csound->GetTieFlag(csound);
}
#endif
...
public:
    ...
    {
#if defined(CSOUND_VERSION_MAJOR) && (CSOUND_VERSION_MAJOR >= 7)
        // In Csound 7, use the opcode deinit entry point instead of the old
        // RegisterDeinitCallback/GetTieFlag/GetReinitFlag helpers.
        return reinterpret_cast<T *>(opcode)->init(csound);
#else
        if (!csound_opcode_reinit_flag(csound) && !csound_opcode_tie_flag(csound)) {
    ...
    {
#if defined(CSOUND_VERSION_MAJOR) && (CSOUND_VERSION_MAJOR >= 7)
        // On Csound 7, this should be used as the opcode deinit callback.
        // It is called by the opcode registration's deinit slot, not by
