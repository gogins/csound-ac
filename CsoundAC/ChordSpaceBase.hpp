...
#define CHORD_SPACE_DEBUG                \
    ...
    public:
         ...
         * equivalent is not found in the indicated opt_sector, an equivalent 
         ...
         * the equivalent is not found in the indicated opt_sector, an equivalent 
         * from another sector is returned.
         */
         ...
         * in the indicated opt_sector, an equivalent from another sector is 
         * returned.
         */
         ...
         * equivalence. If the equivalent is not found in the indicated opt_sector,
         ...
         * equivalent is not found in the indicated opt_sector, an equivalent from
        ...
        /**
         * Returns the equivalent of the chord within the representative fundamental
         * domain of range, transpositional, and inversional equivalence.
         */
        virtual Chord eRTI(double range, int opt_sector) const;
        virtual Chord eRTTI(double range, double g = 1., int opt_sector = 0) const;
        /**
         * Returns the equivalent of the chord within a fundamental
         * domain of range, permutational, and inversional equivalence.
         * If the equivalent is not found in the indicated opt_sector, an equivalent
         * from another sector is returned.
         */
         ...
         * is not found in the indicated opt_sector, an equivalent from another sector
         * is returned.
         */
         ...
         * Returns the equivalent of the chord within the representative fundamental
         * domain of range, permutational, transpositional, and inversional
         * equivalence. If the equivalent is not found in the indicated opt_sector, an equivalent
         * from another sector is returned. 
         */
         ...
         * domain of range, transpositional, and inversional equivalence.
         */
        virtual bool iseRTI(double range, int opt_sector) const;
        /**
       ...
       /**
         * Returns whether the chord is within a fundamental domain of range,
         * transpositional, and inversional equivalence in the equal 
         * temperament generated by g.
         */
        virtual bool iseRTTI(double range, double g = 1., int opt_sector = 0) const;
        /**
         * Returns whether the chord is within the representative fundamental
    ...
    // CANONICALIZERS FOR EQUIVALENCE RELATIONS

    inline SILENCE_PUBLIC bool chord_equal_tolerance(const Chord &a, const Chord &b)
    {
        ...
        {
            if (!eq_tolerance(a.getPitch(v), b.getPitch(v)))
            {
                return false;
            }
        }
    ...
    inline SILENCE_PUBLIC Chord sort_chord_ascending(const Chord &chord)
    {
        ...
        pitches.reserve(static_cast<size_t>(out.voices()));
        for (int v = 0; v < out.voices(); ++v)
        {
            pitches.push_back(out.getPitch(v));
        }
        std::sort(pitches.begin(), pitches.end());
        for (int v = 0; v < out.voices(); ++v)
        {
            out.setPitch(v, pitches[static_cast<size_t>(v)]);
        }
        return out;
    }

    inline SILENCE_PUBLIC int default_range_lift_bound_for_voices(int voices)
    {
        // Keep enumeration small; scales to 12 voices.
        if (voices <= 4)
        {
            return 2;
        }
    ...
    inline SILENCE_PUBLIC void enumerate_k_impl(Eigen::VectorXi &k, int index, int bound, const Fn &fn)
    {
        if (index >= k.size())
        {
            fn(k);
        ...
        for (int val = -bound; val <= bound; ++val)
        {
            k(index) = val;
            enumerate_k_impl(k, index + 1, bound, fn);
        }
    }

    template <typename Fn>
    inline SILENCE_PUBLIC void enumerate_k(int voices, int bound, const Fn &fn)
    {
        if (voices <= 0)
        {
            return;
        ...
        // Gauge-fix: k[0] = 0, enumerate remaining voices.
        Eigen::VectorXi k(voices);
        k.setZero();
        ...
        enumerate_k_impl(k, 1, bound, fn);
    }

    inline SILENCE_PUBLIC Chord apply_range_lift_to_chord(const Chord &chord, const Eigen::VectorXi &k, double range)
    {
        Chord out = chord;
        ...
        for (int v = 0; v < n; ++v)
        {
            ...
            const double lifted = p + static_cast<double>(k(v)) * range;
            out.setPitch(v, lifted);
        }
        return out;
    }

    inline SILENCE_PUBLIC double chord_score(const Chord &chord)
    {
        // Deterministic scalar score: squared norm of the pitch vector.
        const auto v = chord.col(0).eval();
        return static_cast<double>(v.squaredNorm());
    }

    inline SILENCE_PUBLIC bool chord_lex_less(const Chord &a, const Chord &b)
    {
    ...
    inline SILENCE_PUBLIC bool chord_better(const Chord &a, const Chord &b)
    {
        const double sa = chord_score(a);
        const double sb = chord_score(b);

        if (!eq_tolerance(sa, sb))
        {
            return sa < sb;
        }
        return chord_lex_less(a, b);
    }

    template <typename MakeCandidateFn>
    inline SILENCE_PUBLIC Chord canonicalize_by_range_lifts(
        const Chord &chord,
        double range,
        MakeCandidateFn make_candidate)
    {
        bool found = false;
        Chord best;

        const int n = chord.voices();
        const int bound = default_range_lift_bound_for_voices(n);

        enumerate_k(n, bound, [&](const Eigen::VectorXi &k)
        {
            const Chord lifted = apply_range_lift_to_chord(chord, k, range);
            const Chord cand = make_candidate(lifted);
        ...
        return make_candidate(chord);
    }

    template <typename CanonNoIFn>
    inline SILENCE_PUBLIC Chord canonicalize_with_inversion(
        const Chord &chord,
        double range,
        double g,
        CanonNoIFn canon_no_i)
    {
        const Chord a = canon_no_i(chord);

        // Since we are ignoring opt_sector, fold over all inversion flats and pick best.
        bool found = false;
        Chord best;
        ...
        for (int s = 0; s < n; ++s)
        {
            const Chord r = reflect_in_inversion_flat(chord, s);
            const Chord b = canon_no_i(r);
            consider(b);
        }
    ...
    */
    ...
    template <>
    inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_T>(const Chord &chord, double range, double g, int opt_sector)
    {
    ...
    template <>
    inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RP>(const Chord &chord, double range, double g, int opt_sector)
    {
...
inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPT>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord rep = equate<EQUIVALENCE_RELATION_RPT>(chord, range, g, 0);
...
inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPT>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    Chord c = equate<EQUIVALENCE_RELATION_R>(chord, range, 1.0, 0);
    c = equate<EQUIVALENCE_RELATION_P>(c, range, 1.0, 0);
    c = equate<EQUIVALENCE_RELATION_T>(c, range, 1.0, 0);
    c = sort_chord_ascending(c);
    return c;
}
...
inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPTg>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord rep = equate<EQUIVALENCE_RELATION_RPTg>(chord, range, g, 0);
...
inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPTg>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    Chord c = equate<EQUIVALENCE_RELATION_R>(chord, range, 1.0, 0);
    c = equate<EQUIVALENCE_RELATION_P>(c, range, 1.0, 0);
    c = equate<EQUIVALENCE_RELATION_Tg>(c, range, g, 0);
    c = sort_chord_ascending(c);
    return c;
}
...
inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord rep = equate<EQUIVALENCE_RELATION_RPI>(chord, range, g, 0);
...
inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord a = equate<EQUIVALENCE_RELATION_RP>(chord, range, 1.0, 0);

    Chord b = reflect_in_inversion_flat(a, 0);
    b = equate<EQUIVALENCE_RELATION_RP>(b, range, 1.0, 0);
...
inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RTI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord rep = equate<EQUIVALENCE_RELATION_RTI>(chord, range, g, 0);
...
inline bool Chord::iseRTI(double range, int opt_sector) const
{
    return predicate<EQUIVALENCE_RELATION_RTI>(*this, range, 1.0, opt_sector);
}
...
inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RTI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord a = equate<EQUIVALENCE_RELATION_RT>(chord, range, 1.0, 0);

    Chord b = reflect_in_inversion_flat(a, 0);
    b = equate<EQUIVALENCE_RELATION_RT>(b, range, 1.0, 0);

    return (b < a) ? b : a;
}

inline Chord Chord::eRTI(double range, int opt_sector) const
{
    return csound::equate<EQUIVALENCE_RELATION_RTI>(*this, range, 1.0, opt_sector);
}
...
inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RTgI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord rep = equate<EQUIVALENCE_RELATION_RTgI>(chord, range, g, 0);
...
inline bool Chord::iseRTTI(double range, double g, int opt_sector) const
{
    return predicate<EQUIVALENCE_RELATION_RTgI>(*this, range, g, opt_sector);
}
...
inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RTgI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord a = equate<EQUIVALENCE_RELATION_RT>(chord, range, 1.0, 0);
    const Chord aq = equate<EQUIVALENCE_RELATION_Tg>(a, range, g, 0);

    Chord b = reflect_in_inversion_flat(aq, 0);
    b = equate<EQUIVALENCE_RELATION_Tg>(b, range, g, 0);

    return (b < aq) ? b : aq;
}

inline Chord Chord::eRTTI(double range, double g, int opt_sector) const
{
    return csound::equate<EQUIVALENCE_RELATION_RTgI>(*this, range, g, opt_sector);
}
...
inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPTI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord rep = equate<EQUIVALENCE_RELATION_RPTI>(chord, range, g, 0);
...
inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPTI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord a = equate<EQUIVALENCE_RELATION_RPT>(chord, range, 1.0, 0);

    Chord b = reflect_in_inversion_flat(a, 0);
    b = equate<EQUIVALENCE_RELATION_RPT>(b, range, 1.0, 0);
...
inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPTgI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord rep = equate<EQUIVALENCE_RELATION_RPTgI>(chord, range, g, 0);
...
inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPTgI>(const Chord &chord, double range, double g, int opt_sector)
{
    ...
    const Chord a = equate<EQUIVALENCE_RELATION_RPTg>(chord, range, g, 0);

    Chord b = reflect_in_inversion_flat(a, 0);
    b = equate<EQUIVALENCE_RELATION_RPTg>(b, range, g, 0);
