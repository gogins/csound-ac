...
#define CHORD_SPACE_DEBUG                \
    ...
    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_P>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_T>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_Tg>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_I>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RP>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RT>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPT>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPTg>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPI>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RTI>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RTgI>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPTI>(const Chord &chord, double range, double g, int opt_sector);

    template <>
    SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPTgI>(const Chord &chord, double range, double g, int opt_sector);
    ...
    template <>
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RT>(const Chord &, double, double, int);
    template <>
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_R>(const Chord &, double, double, int);
    template <>
    ...
    {
        (void)opt_sector;
        const int canonical_opt_sector = 0;

        if (!predicate<EQUIVALENCE_RELATION_R>(chord, range, g, canonical_opt_sector))
        {
            return false;
        }
        if (!predicate<EQUIVALENCE_RELATION_T>(chord, range, g, canonical_opt_sector))
        {
            return false;
        }
        return predicate<EQUIVALENCE_RELATION_I>(chord, range, g, canonical_opt_sector);
    }
    ...
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RTI>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        Chord c = equate<EQUIVALENCE_RELATION_RT>(chord, range, g, canonical_opt_sector);

        if (!predicate<EQUIVALENCE_RELATION_I>(c, range, g, canonical_opt_sector))
        {
            c = reflect_in_inversion_flat(c, canonical_opt_sector);
            c = equate<EQUIVALENCE_RELATION_T>(c, range, g, canonical_opt_sector);
        }
    ...
    inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RTgI>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        if (!predicate<EQUIVALENCE_RELATION_Tg>(chord, range, g, canonical_opt_sector))
        {
            return false;
        }
        return predicate<EQUIVALENCE_RELATION_I>(chord, range, g, canonical_opt_sector);
    ...
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RTgI>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        Chord c = equate<EQUIVALENCE_RELATION_R>(chord, range, g, canonical_opt_sector);
        c = equate<EQUIVALENCE_RELATION_Tg>(c, range, g, canonical_opt_sector);
    ...
    */
    ...
    inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPT>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        if (!predicate<EQUIVALENCE_RELATION_P>(chord, range, g, canonical_opt_sector))
        {
            return false;
        }
        if (!predicate<EQUIVALENCE_RELATION_T>(chord, range, g, canonical_opt_sector))
        ...
        return chord.is_opt_sector(canonical_opt_sector);
    }
    ...
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPT>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        Chord c = equate<EQUIVALENCE_RELATION_R>(chord, range, g, canonical_opt_sector);
        c = equate<EQUIVALENCE_RELATION_P>(c, range, g, canonical_opt_sector);
        c = equate<EQUIVALENCE_RELATION_T>(c, range, g, canonical_opt_sector);
        return c;
    }
    ...
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPI>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        Chord c = equate<EQUIVALENCE_RELATION_RP>(chord, range, g, canonical_opt_sector);
    ...
    inline SILENCE_PUBLIC bool predicate<EQUIVALENCE_RELATION_RPTI>(const Chord &chord, double range, double g, int opt_sector)
    {
        (void)opt_sector;
        const int canonical_opt_sector = 0;
        const int canonical_opti_sector = 0;
        ...
        return chord.is_opti_sector(canonical_opti_sector);
    }
    ...
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPTI>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        Chord c = equate<EQUIVALENCE_RELATION_RPT>(chord, range, g, canonical_opt_sector);

        if (!c.is_opti_sector(canonical_opti_sector))
        {
            c = reflect_in_inversion_flat(c, canonical_opt_sector);
            c = equate<EQUIVALENCE_RELATION_P>(c, range, g, canonical_opt_sector);
            c = equate<EQUIVALENCE_RELATION_T>(c, range, g, canonical_opt_sector);
    ...
    inline SILENCE_PUBLIC Chord equate<EQUIVALENCE_RELATION_RPTgI>(const Chord &chord, double range, double g, int opt_sector)
    {
        ...
        Chord c = equate<EQUIVALENCE_RELATION_RPTg>(chord, range, g, canonical_opt_sector);
